...
class TelegramPresenter:
    ...
    def _makeInlineGameArticle(game: GameResultVM, countryConfig: CountryConfig):
        keyboardMarkup = TelegramPresenter._makeKeyboardMarkup(
            appid=game.appid, steamlink=game.link, hasProtonDB=game.protonDB is not None
        )
    ...
    @staticmethod
    def _makeSpecialInlineQueryResult(
    ...
    def _makeInlineQueryResultsList(games: SearchResults, countryConfig: CountryConfig):
        articles = [
            TelegramPresenter._makeInlineGameArticle(game, countryConfig).queryArticle
    ...
    @staticmethod
    def makeInlineQueryPresentation(
        searchResults: SearchResults, countryConfig: CountryConfig
    ) -> TelegramInlineResultListPres:
        return TelegramPresenter._makeInlineQueryResultsList(
            searchResults, countryConfig
        )
    ...
    def _makeCountryKeyboard(codes: list[str]):
        ...
        for i in range(0, len(codes), 3):
            row = [
                InlineKeyboardButton(
                    code, callback_data=TelegramCallbackBuilder.setcurrency(code)
                )
    ...
    def makeCurrencyMessageFromCountry(countryMod: CountryModification):
        if countryMod.configuredCountry or countryMod.requestedCountry:
            if countryMod.configuredCountry:
                ...
                kb = TelegramPresenter._makeCountryKeyboard(
                    countryMod.alternativeSuggestions
                )
        else:
            text = (
    ...
    def _makeKeyboardMarkup(appid, steamlink, hasProtonDB: bool):
        row1buttons = [InlineKeyboardButton("Steam Page", url=steamlink)]
