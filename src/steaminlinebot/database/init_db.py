...
GAME_SOURCE_SEED_PATH = Path(__file__).resolve().parent.parent.parent.parent / "data" / "shop_ids_current.json"
COUNTRY_SEED_PATH = Path(__file__).resolve().parent.parent.parent.parent / "data" / "countries.json"
...
def _ensure_schema_version(engine: Engine) -> int:
    ...
    with engine.begin() as conn:
        ...
        result = conn.execute(text("SELECT COALESCE(MAX(version), -1) FROM schema_version"))
...
def _migration_000_seed_countries(engine: Engine) -> None:
    ...
    rows = [{"alpha2": c["code"]} for c in data["countries"]]

    with engine.begin() as conn:
        stmt = sqlite_insert(country_table).on_conflict_do_nothing(index_elements=["alpha2"])
...
def _migration_001_seed_game_sources(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        stmt = sqlite_insert(game_source_table).on_conflict_do_nothing(index_elements=["itad_shop_id"])
        conn.execute(stmt, rows)
...
MIGRATIONS: list[tuple[int, str, Callable[[Engine], None] | str]] = [
    ...
    (1, "seed game_source from shop_ids_current.json", _migration_001_seed_game_sources),
]
